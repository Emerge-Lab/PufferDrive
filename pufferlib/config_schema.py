...
class DriveEnvConfig:
    ...
    target_obs_slots_partners_n: int = _constrained_field(NONNEGATIVE_INT_CONSTRAINT)
    ...
    target_obs_range_partner_m: float = _constrained_field(NONNEGATIVE_NUMBER_CONSTRAINT)
...
class TrainingConfig:
    ...
    target_policy: str | None = MISSING
...
def _validate_cross_field_constraints(config, context):
    ...
    if not PDM_MIN_HORIZON_SECONDS <= env["pdm_horizon"] <= PDM_MAX_HORIZON_SECONDS:
        _raise_config_error(
            context,
            "env.pdm_horizon",
            f"must be in [{PDM_MIN_HORIZON_SECONDS}, {PDM_MAX_HORIZON_SECONDS}]",
        )
    if not PDM_MIN_PLANNING_DT_SECONDS <= env["pdm_planning_dt"] <= PDM_MAX_PLANNING_DT_SECONDS:
        _raise_config_error(
            context,
            "env.pdm_planning_dt",
            f"must be in [{PDM_MIN_PLANNING_DT_SECONDS}, {PDM_MAX_PLANNING_DT_SECONDS}]",
        )
    if env["target_obs_slots_partners_n"] > env["obs_slots_partners_n"]:
        _raise_config_error(
            context,
            "env.target_obs_slots_partners_n",
            "must not exceed env.obs_slots_partners_n",
        )
    if env["target_obs_range_partner_m"] > env["obs_range_partner_m"]:
        _raise_config_error(
            context,
            "env.target_obs_range_partner_m",
            "must not exceed env.obs_range_partner_m",
        )
...
def validate_puffer_drive_resources(config, context):
    ...
    target_policy_path = config["train"].get("target_policy")
    if target_policy_path is not None and not os.path.isfile(target_policy_path):
