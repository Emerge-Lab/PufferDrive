...
def main():
    ...
    print(f"points per polyline: min={polylines['lengths'].min()}, max={polylines['lengths'].max()}, mean={polylines['lengths'].mean():.1f}")
    ...
    for i, idx in enumerate(poly_indices):
        ...
        if len(x) >= 2:
            ...
            dx = x[mid] - x[mid-1]
            dy = y[mid] - y[mid-1]
            ax.annotate('', xy=(x[mid], y[mid]), xytext=(x[mid-1], y[mid-1]),
