...
static int handler(
    ...
    if (MATCH("env", "action_type")) {
        ...
        if(env_config->num_agents_per_world <= 0 && env_config->init_mode == RANDOM_AGENTS_INIT) {
            raise_error_with_message(ERROR_INVALID_CONFIG, "num_agents_per_world must be positive for random_agents_init init_mode");
        }
