...
DriveNet* init_drivenet(Weights* weights, int num_agents, bool use_rc, bool use_ec, bool use_dc, bool oracle_mode) {
    ...
    if (oracle_mode && net->conditioning_dims > 0) {
        // NOTE: Python hardcodes oracle to 64 agents (see drive.py:101)
        net->obs_oracle = calloc(num_agents*64*net->conditioning_dims, sizeof(float)); // Each agent receives conditioning from 64 agents
        net->oracle_linear_output = calloc(num_agents*64*input_size, sizeof(float));
        net->oracle_linear_output_two = calloc(num_agents*64*input_size, sizeof(float));
        net->oracle_layernorm_output = calloc(num_agents*64*input_size, sizeof(float));
        ...
        net->oracle_max = make_max_dim1(num_agents, 64, input_size);
    }
...
void forward(DriveNet* net, float* observations, int* actions) {
    ...
    // NOTE: Python hardcodes oracle to 64 agents (see drive.py:101)
    int oracle_obs_size = net->oracle_mode ? 64 * net->conditioning_dims : 0;
    ...
    if (net->oracle_mode) {
        memset(net->obs_oracle, 0, net->num_agents * 64 * net->conditioning_dims * sizeof(float));
    }
    ...
    float (*obs_oracle)[64][net->conditioning_dims] = net->oracle_mode ? (float (*)[64][net->conditioning_dims])net->obs_oracle : NULL;
    ...
    for (int b = 0; b < net->num_agents; b++) {
        ...
        if (net->oracle_mode) {
            // Python hardcodes oracle to 64 agents
            for(int i = 0; i < 64; i++) {
    ...
    if (net->oracle_mode) {
        ...
        for (int b = 0; b < net->num_agents; b++) {
            for (int obj = 0; obj < 64; obj++) {
                float* obj_features = &net->obs_oracle[b*64*net->conditioning_dims + obj*net->conditioning_dims];
                _linear(obj_features, net->oracle_encoder->weights, net->oracle_encoder->bias,
                       &net->oracle_linear_output[b*64*64 + obj*64], 1, net->conditioning_dims, 64);
            }
        }

        for (int b = 0; b < net->num_agents; b++) {
            for (int obj = 0; obj < 64; obj++) {
                float* after_first = &net->oracle_linear_output[b*64*64 + obj*64];
                _layernorm(after_first, net->oracle_layernorm->weights, net->oracle_layernorm->bias,
                          &net->oracle_layernorm_output[b*64*64 + obj*64], 1, 64);
            }
        }

        for (int b = 0; b < net->num_agents; b++) {
            for (int obj = 0; obj < 64; obj++) {
                float* after_first = &net->oracle_layernorm_output[b*64*64 + obj*64];
                _linear(after_first, net->oracle_encoder_two->weights, net->oracle_encoder_two->bias,
                        &net->oracle_linear_output_two[b*64*64 + obj*64], 1, 64, 64);
            }
        }
