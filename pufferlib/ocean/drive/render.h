...
void draw_agent_obs(Drive *env, int agent_index, int mode, int obs_only, int lasers) {
    ...
    int target_count = env->num_target_waypoints;
    int target_dim = (env->target_type == TARGET_STATIC) ? target_count * STATIC_TARGET_FEATURES
                                                         : target_count * DYNAMIC_TARGET_FEATURES;
    ...
    if (target_count > 0) {
        ...
        float target_scale
            = (env->target_type == TARGET_STATIC) ? env->obs_norm_goal_offset_m : env->obs_norm_xy_offset_m;
        ...
        Color target_color = (env->target_type == TARGET_STATIC) ? LIGHTGREEN : SKYBLUE;
        DrawSphere((Vector3) {draw_x, draw_y, 1}, 0.5f, target_color);
    ...
    int obs_idx = ego_dim + num_reward_coefs + target_dim; // Start after ego, conditioning, and target obs
...
void draw_scene(Drive *env, Client *client, int mode, int obs_only, int lasers, int show_grid) {
    ...
    for (int i = 0; i < env->num_total_agents; i++) {
        ...
        if (mode == 1) {
            ...
            if (agent_index == EGO_IDX && !agent->metrics_array[REACHED_GOAL_IDX]) {
                draw_agent_obs(env, agent_index, mode, obs_only, lasers);
            }
            if ((obs_only || IsKeyDown(KEY_LEFT_CONTROL)) && agent_index != EGO_IDX) {
                continue;
            }
            ...
            DrawModelEx(car_model, (Vector3) {0, 0, 0}, (Vector3) {1, 0, 0}, 90.0f, scale, WHITE);
            {
                ...
                Vector3 corners[4] = {
                    (Vector3) {0 + (half_len * cos_heading - half_width * sin_heading),
                               0 + (half_len * sin_heading + half_width * cos_heading),
                               0},
                    (Vector3) {0 + (half_len * cos_heading + half_width * sin_heading),
                               0 + (half_len * sin_heading - half_width * cos_heading),
                               0},
        ...
        // FPV Camera Control
        if (IsKeyDown(KEY_SPACE) && EGO_IDX == agent_index) {
        ...
        if (!IsKeyDown(KEY_LEFT_CONTROL) && obs_only == 0) {
            ...
            DrawCircle3D(
                (Vector3) {agent->goal_position_x, agent->goal_position_y, 0.1f},
                env->goal_radius,
                (Vector3) {0, 0, 1},
                90.0f,
                Fade(LIGHTGREEN, 0.3f));
...
void saveAgentViewImage(
    ...
    int agent_idx = env->active_agent_indices[EGO_IDX];
...
void c_render(Drive *env) {
    ...
    int ego_idx = env->active_agent_indices[EGO_IDX];
    DrawText(TextFormat("Controlling Agent: %d", EGO_IDX), 10, 70, 20, PUFF_WHITE);
    ...
    if (env->action_type == 1) { // continuous (float)
        ...
        DrawText(TextFormat("Acceleration: %.2f", action_array_f[EGO_IDX][0]), 10, 110, 20, PUFF_WHITE);
        DrawText(TextFormat("Steering: %.2f", action_array_f[EGO_IDX][1]), 10, 130, 20, PUFF_WHITE);
    } else { // discrete (int)
        int (*action_array)[2] = (int (*)[2]) env->actions;
        DrawText(TextFormat("Acceleration: %d", action_array[EGO_IDX][0]), 10, 110, 20, PUFF_WHITE);
        DrawText(TextFormat("Steering: %d", action_array[EGO_IDX][1]), 10, 130, 20, PUFF_WHITE);
    }
