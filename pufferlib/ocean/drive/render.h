...
void draw_scene(Drive *env, Client *client, int mode, int obs_only, int lasers, int show_grid) {
    ...
    for (int i = 0; i < env->num_total_agents; i++) {
        ...
        bool is_expert = (!is_active_agent) && (agent->control_state == CONTROL_STATE_MOVING);
        ...
        if (mode == 1) {
            ...
            DrawModelEx(car_model, (Vector3) {0, 0, 0}, (Vector3) {1, 0, 0}, 90.0f, scale, WHITE);
            {
                ...
                if (!is_active_agent && agent->control_state == CONTROL_STATE_MOVING) {
        ...
        if (!IsKeyDown(KEY_LEFT_CONTROL) && obs_only == 0) {
            DrawSphere((Vector3) {agent->current_goal_x, agent->current_goal_y, 1}, 0.5f, DARKGREEN);

            DrawCircle3D(
                (Vector3) {agent->current_goal_x, agent->current_goal_y, 0.1f},
                env->goal_radius,
                (Vector3) {0, 0, 1},
                90.0f,
                Fade(LIGHTGREEN, 0.3f));
