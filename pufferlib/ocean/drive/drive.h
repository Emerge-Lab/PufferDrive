...
static float normalize_reward_coef(float value, int coef_idx) {
    ...
    if (!(coef_idx == REWARD_COEF_GOAL_RADIUS || coef_idx == REWARD_COEF_LANE_ALIGN ||
          coef_idx == REWARD_COEF_LANE_CENTER || coef_idx == REWARD_COEF_VEL_ALIGN ||
          coef_idx == REWARD_COEF_CENTER_BIAS)) {
        return 0;
    }
    //
