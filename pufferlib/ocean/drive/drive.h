...
#define MIN_POINTS_IN_RADIUS_TO_SAMPLE_GOAL 5
...
#define GRID_MAP_CACHE_VISION_RANGE 21      // Vision range to cache neighbor offsets
...
struct Entity {
    ...
    float next_goal_position_x;
    float next_goal_position_y;
    float init_goal_x;
    float init_goal_y;
    int mark_as_expert;
    int collision_state;
...
struct Drive {
    ...
    float goal_radius;  // Distance threshold to consider goal reached
...
// Checks for collisions at a specified spawn point
bool is_valid_spawn_point(Drive* env, int agent_idx, float start_x, float start_y, float start_z, float heading, float vehicle_width, float vehicle_length, float vehicle_height) {
    // Create Mock Entity for collision checking
    Entity mock_agent = {
        .x = start_x,
        .y = start_y,
        .z = start_z,
        ...
        .heading = heading,
        .heading_x = cosf(heading),
        .heading_y = sinf(heading)
    };

    float half_length = mock_agent.length/2.0f;
    float half_width = mock_agent.width/2.0f;
    float cos_heading = cosf(mock_agent.heading);
    float sin_heading = sinf(mock_agent.heading);
    ...
    for (int i = 0; i < env->num_entities; i++) {
        ...
        if (entity->type != VEHICLE && entity-> type != PEDESTRIAN && entity->type != CYCLIST) continue; // Only check dynamic entities
        if (entity->id == agent_idx) continue; // Skip self
...
Entity* load_map_binary(const char* filename, Drive* env) {
    ...
    if (env->init_mode == INIT_RANDOM_AGENTS) {
        ...
        for (int i = 0; i < num_bin_entities; i++) {
            ...
            fread(&read_bin_entities[i].scenario_id, sizeof(int), 1, file);
            fread(&read_bin_entities[i].type, sizeof(int), 1, file);
            fread(&read_bin_entities[i].id, sizeof(int), 1, file);
            fread(&read_bin_entities[i].array_size, sizeof(int), 1, file);
            // Allocate arrays based on type
            int size = read_bin_entities[i].array_size;
            read_bin_entities[i].traj_x = (float*)malloc(size * sizeof(float));
            read_bin_entities[i].traj_y = (float*)malloc(size * sizeof(float));
            read_bin_entities[i].traj_z = (float*)malloc(size * sizeof(float));
            if (read_bin_entities[i].type == VEHICLE || read_bin_entities[i].type == PEDESTRIAN || read_bin_entities[i].type == CYCLIST) {  // Object type
                // Allocate arrays for object-specific data
                read_bin_entities[i].traj_vx = (float*)malloc(size * sizeof(float));
                read_bin_entities[i].traj_vy = (float*)malloc(size * sizeof(float));
                read_bin_entities[i].traj_vz = (float*)malloc(size * sizeof(float));
                read_bin_entities[i].traj_heading = (float*)malloc(size * sizeof(float));
                read_bin_entities[i].traj_valid = (int*)malloc(size * sizeof(int));
            } else {
                // Roads don't use these arrays
                read_bin_entities[i].traj_vx = NULL;
                read_bin_entities[i].traj_vy = NULL;
                read_bin_entities[i].traj_vz = NULL;
                read_bin_entities[i].traj_heading = NULL;
                read_bin_entities[i].traj_valid = NULL;
            }
            // Read array data
            fread(read_bin_entities[i].traj_x, sizeof(float), size, file);
            fread(read_bin_entities[i].traj_y, sizeof(float), size, file);
            fread(read_bin_entities[i].traj_z, sizeof(float), size, file);
            if (read_bin_entities[i].type == VEHICLE || read_bin_entities[i].type == PEDESTRIAN || read_bin_entities[i].type == CYCLIST) {  // Object type
                fread(read_bin_entities[i].traj_vx, sizeof(float), size, file);
                fread(read_bin_entities[i].traj_vy, sizeof(float), size, file);
                fread(read_bin_entities[i].traj_vz, sizeof(float), size, file);
                fread(read_bin_entities[i].traj_heading, sizeof(float), size, file);
                fread(read_bin_entities[i].traj_valid, sizeof(int), size, file);
            }
            // Read remaining scalar fields
            fread(&read_bin_entities[i].width, sizeof(float), 1, file);
            fread(&read_bin_entities[i].length, sizeof(float), 1, file);
            fread(&read_bin_entities[i].height, sizeof(float), 1, file);
            fread(&read_bin_entities[i].goal_position_x, sizeof(float), 1, file);
            fread(&read_bin_entities[i].goal_position_y, sizeof(float), 1, file);
            fread(&read_bin_entities[i].goal_position_z, sizeof(float), 1, file);
            fread(&read_bin_entities[i].mark_as_expert, sizeof(int), 1, file);
        ...
        int env_entity_idx = 0;
        for (int i = 0; i < num_bin_entities; i++) {
            if (read_bin_entities[i].type == VEHICLE || read_bin_entities[i].type == PEDESTRIAN || read_bin_entities[i].type == CYCLIST) {
                continue;
            } else {
                entities[env_entity_idx] = read_bin_entities[i];
                env_entity_idx++;
            }
        }
        ...
        while (env_entity_idx < env->num_entities) {
            ...
            entities[env_entity_idx].id = env_entity_idx;
            entities[env_entity_idx].initialized = 0;
            entities[env_entity_idx].array_size = 0;
            entities[env_entity_idx].traj_x = NULL;
            entities[env_entity_idx].traj_y = NULL;
            entities[env_entity_idx].traj_z = NULL;
            entities[env_entity_idx].traj_vx = NULL;
            entities[env_entity_idx].traj_vy = NULL;
            entities[env_entity_idx].traj_vz = NULL;
            entities[env_entity_idx].traj_heading = NULL;
            entities[env_entity_idx].traj_valid = NULL;
            env_entity_idx++;
        }
        ...
        env->num_entities = num_bin_entities;
        Entity* entities = (Entity*)malloc(env->num_entities * sizeof(Entity));
        for (int i = 0; i < env->num_entities; i++) {
            // Read base entity data
...
void init_agents_random_start(Drive* env) {
    ...
    for (int agent_idx=0; agent_idx<env->num_entities; agent_idx++){
        ...
        // Random Initializing Agent
        while(1){
            // Random Road lane of a random grid cell
            int rand_grid_col = rand() % env->grid_map->grid_cols;
            int rand_grid_row = rand() % env->grid_map->grid_rows;
            int grid_map_idx = rand_grid_row * env->grid_map->grid_cols + rand_grid_col;
            // If cell has no lanes, repeat
            if (env->grid_map->cell_roadlanes_count[grid_map_idx] == 0){
                continue;
            }
            int cell_lane_count = env->grid_map->cell_roadlanes_count[grid_map_idx];
            int lane_idx = rand() % cell_lane_count;
            // float heading = ((float)rand() / RAND_MAX) * 2.0f * PI;     // Random heading
            float heading = 0.0f;

            // Get start lane point info
            float start_x = INVALID_POSITION, start_y = INVALID_POSITION, start_z = INVALID_POSITION, start_heading = INVALID_POSITION;
            int cnt = 0;
            for (int i = 0; i < env->grid_map->cell_entities_count[grid_map_idx]; i++) {
                int entity_idx = env->grid_map->cells[grid_map_idx][i].entity_idx;
                ...
                // Current spawn point
                if (cnt == lane_idx) {
                    int start_point_geometry_idx = env->grid_map->cells[grid_map_idx][i].geometry_idx;
                    if (start_point_geometry_idx + 1 < entity.array_size) {
                        float dx = entity.traj_x[start_point_geometry_idx + 1] - entity.traj_x[start_point_geometry_idx];
                        float dy = entity.traj_y[start_point_geometry_idx + 1] - entity.traj_y[start_point_geometry_idx];
                        heading = atan2f(dy, dx);
                    }
                    else if (start_point_geometry_idx - 1 >= 0) {
                        float dx = entity.traj_x[start_point_geometry_idx] - entity.traj_x[start_point_geometry_idx - 1];
                        float dy = entity.traj_y[start_point_geometry_idx] - entity.traj_y[start_point_geometry_idx - 1];
                        heading = atan2f(dy, dx);
                    }
                    else {
                    ...
                    bool valid_spawn = is_valid_spawn_point(
                        env,
                        agent.id,
                        entity.traj_x[start_point_geometry_idx],
                    ...
                    if (valid_spawn) {
                        ...
                        start_y = entity.traj_y[start_point_geometry_idx];
                        start_z = entity.traj_z[start_point_geometry_idx];
                        start_heading = heading;
                        break;
                    }

                    break;  // Break anyways as current spawn pt is invalid
                }
                cnt++;
            }
            // Invalid start point, repeat
            if (start_x == INVALID_POSITION || start_y == INVALID_POSITION || start_z == INVALID_POSITION) {
                continue;
            }
            ...
            agent.traj_z = (float*)calloc(agent.array_size, sizeof(float));
            agent.traj_vx = (float*)calloc(agent.array_size, sizeof(float));
            agent.traj_vy = (float*)calloc(agent.array_size, sizeof(float));
            agent.traj_vz = (float*)calloc(agent.array_size, sizeof(float));
            agent.traj_heading = (float*)calloc(agent.array_size, sizeof(float));
            agent.traj_valid = (int*)calloc(agent.array_size, sizeof(int));
            agent.traj_x[0] = start_x;
            agent.traj_y[0] = start_y;
            agent.traj_z[0] = start_z;
            agent.x = start_x;
            agent.y = start_y;
            agent.z = start_z;
            agent.traj_heading[0] = start_heading;
            agent.heading = start_heading;
...
void align_heading_to_lane(Drive* env, int agent_idx) {
    ...
    if (lane_idx < 0 || lane_idx >= env->num_entities) {
        return;
    }
    ...
    for (int i = 1; i < lane->array_size; i++) {
        float px = lane->traj_x[i - 1];
        float py = lane->traj_y[i - 1];
        float qx = lane->traj_x[i];
        float qy = lane->traj_y[i];
        float seg_dx = qx - px;
        float seg_dy = qy - py;
        float seg_len2 = seg_dx * seg_dx + seg_dy * seg_dy;
        if (seg_len2 < 1e-6f) continue;

        float t = ((ax - px) * seg_dx + (ay - py) * seg_dy) / seg_len2;
        if (t < 0.0f) t = 0.0f;
        else if (t > 1.0f) t = 1.0f;

        float cx = px + t * seg_dx;
        float cy = py + t * seg_dy;
        float dx = ax - cx;
        float dy = ay - cy;
    ...
    if (best_seg >= 1) {
        float dx = lane->traj_x[best_seg] - lane->traj_x[best_seg - 1];
        float dy = lane->traj_y[best_seg] - lane->traj_y[best_seg - 1];
        float norm = sqrtf(dx * dx + dy * dy);
        if (norm > 1e-6f) {
            dx /= norm;
            dy /= norm;
            float speed = sqrtf(agent->vx * agent->vx + agent->vy * agent->vy);
            agent->heading = atan2f(dy, dx);
            agent->heading_x = dx;
            agent->heading_y = dy;
            agent->vx = speed * dx;
            agent->vy = speed * dy;
...
void ensure_goal_ahead(Drive* env, int agent_idx, float ref_x, float ref_y, float dir_x, float dir_y) {
    ...
    float gdx = agent->goal_position_x - ref_x;
    float gdy = agent->goal_position_y - ref_y;
    float dot = gdx * dir_x + gdy * dir_y;
    if (dot >= env->goal_radius) {
        return;
    }
    float forward_step = fmaxf(env->goal_radius, env->goal_radius * 2.0f);
    agent->goal_position_x = ref_x + forward_step * dir_x;
    agent->goal_position_y = ref_y + forward_step * dir_y;
}

void record_spawn_distance_to_goal(Drive* env, int agent_idx, int log_idx) {
    if (!env->logs || log_idx < 0 || log_idx >= env->active_agent_count) {
        return;
    }

    Entity* agent = &env->entities[agent_idx];
    if (agent->x == INVALID_POSITION ||
        agent->goal_position_x == INVALID_POSITION ||
        agent->goal_position_y == INVALID_POSITION) {
        return;
    }

    float spawn_distance = relative_distance_2d(
        agent->x,
        agent->y,
    ...
    agent->spawn_distance_sum += spawn_distance;
    agent->spawn_count += 1;
...
void init_topology_graph(Drive* env){
    ...
    for(int i = 0; i < env->num_entities; i++){
        ...
        for(int j = 0; j < env->num_entities; j++){
            ...
            if(distance < 1.0f && heading_diff < 0.1f){                // Add directed edge from i to j (lane i connects to lane j)
...
void set_means(Drive* env) {
    ...
    for (int i = 0; i < env->num_entities; i++) {
        if (env->entities[i].type == VEHICLE || env->entities[i].type == PEDESTRIAN || env->entities[i].type == CYCLIST || env->entities[i].type >= 4) {
            if (env->entities[i].initialized == 0) continue;
            for (int j = 0; j < env->entities[i].array_size; j++) {
...
int check_lane_aligned(Entity* car, Entity* lane, int geometry_idx) {
    ...
    // within relaxed threshold (was 15 degrees)
    return (heading_diff < LANE_ALIGNED_HEADING_THRESHOLD_RAD) ? 1 : 0;
}
...
void compute_agent_metrics(Drive* env, int agent_idx) {
    ...
    for (int i = 0; i < list_size ; i++) {
        ...
        if(entity->type == ROAD_LANE) {
            ...
            if (heading_diff > LANE_HEADING_PENALTY_THRESHOLD_RAD) dist += LANE_HEADING_PENALTY;
    ...
    if (closest_lane_entity_idx == -1 || min_distance > LANE_ALIGN_DISTANCE_THRESHOLD) {
        ...
        int fallback_geom_idx = closest_lane_geometry_idx;
        for (int i = 0; i < list_size; i++) {
            if (entity_list[i].entity_idx == -1) continue;
            if (entity_list[i].entity_idx == agent_idx) continue;
            Entity* entity = &env->entities[entity_list[i].entity_idx];
            if (entity->type != ROAD_LANE) continue;
            ...
            if (dist < fallback_min_distance) {
                fallback_min_distance = dist;
                fallback_lane_idx = entity_list[i].entity_idx;
                fallback_geom_idx = geometry_idx;
            }
        }
        min_distance = fallback_min_distance;
        closest_lane_entity_idx = fallback_lane_idx;
    ...
    if (closest_lane_entity_idx == -1) {
        ...
        int global_lane_idx = -1;
        int global_geom_idx = 0;
        for (int e_idx = 0; e_idx < env->num_entities; e_idx++) {
            Entity* e = &env->entities[e_idx];
            if (e->type != ROAD_LANE || e->array_size < 2) continue;
            for (int g = 0; g < e->array_size - 1; g++) {
                float dist = point_to_segment_distance_2d(
                    agent->x, agent->y, e->traj_x[g], e->traj_y[g], e->traj_x[g + 1], e->traj_y[g + 1]);
                if (dist < global_min) {
                    global_min = dist;
                    global_lane_idx = e_idx;
                    global_geom_idx = g;
                }
            }
        }
        if (global_lane_idx != -1) {
            min_distance = global_min;
            closest_lane_entity_idx = global_lane_idx;
            closest_lane_geometry_idx = global_geom_idx;
        }
    }
    ...
    if (closest_lane_entity_idx == -1) {
        ...
        if (min_distance > LANE_ALIGN_DISTANCE_THRESHOLD) {
            agent->metrics_array[LANE_ALIGNED_IDX] = 0.0f;
        } else {
...
void compute_new_goal(Drive* env, int agent_idx) {
    ...
    while (current_entity != -1) {
        ...
        for (int i = start_idx; i < lane->array_size; i++) {
            ...
            if (start_fraction > 1.0f) start_fraction = 1.0f;

            float available_length = segment_length * (1.0f - start_fraction);

            if (remaining_distance <= available_length) {
            ...
            remaining_distance -= available_length;
        }
        ...
        if (lane->array_size >= 2) {
            ...
            float dx = lane->traj_x[last_idx] - lane->traj_x[last_idx - 1];
            float dy = lane->traj_y[last_idx] - lane->traj_y[last_idx - 1];
            float norm = sqrtf(dx * dx + dy * dy);
            if (norm > 1e-4f) {
                forward_x = dx / norm;
                forward_y = dy / norm;
        ...
        for (int c = 0; c < num_connected; c++) {
            Entity* next_lane = &env->entities[connected_lanes[c]];
            if (next_lane->array_size < 2) {
                continue;
            }
            float dx = next_lane->traj_x[1] - next_lane->traj_x[0];
            float dy = next_lane->traj_y[1] - next_lane->traj_y[0];
            float norm = sqrtf(dx * dx + dy * dy);
            if (norm < 1e-4f) {
                continue;
            }
            ...
            float to_start_y = start_y - agent->y;
            float ahead = to_start_x * forward_x + to_start_y * forward_y;
            if (dot > 0.0f && ahead > 0.0f && dot > best_dot) {
                best_dot = dot;
                best_idx = c;
            }
        }
...
void set_next_goal_preview(Drive* env, int agent_idx) {
    ...
    if (agent->goal_position_x == INVALID_POSITION || agent->goal_position_y == INVALID_POSITION) {
        agent->next_goal_valid = 0;
        return;
    }

    float prev_goal_x = agent->goal_position_x;
    float prev_goal_y = agent->goal_position_y;
    float prev_x = agent->x;
    float prev_y = agent->y;
    float prev_heading = agent->heading;
    float prev_heading_x = agent->heading_x;
    ...
    agent->x = prev_x;
    agent->y = prev_y;
    agent->heading = prev_heading;
    agent->heading_x = prev_heading_x;
    agent->heading_y = prev_heading_y;
...
void promote_next_goal(Drive* env, int agent_idx) {
    ...
    if (agent->next_goal_valid) {
        agent->goal_position_x = agent->next_goal_position_x;
        agent->goal_position_y = agent->next_goal_position_y;
        agent->sampled_new_goal = 0;
    } else {
        compute_new_goal(env, agent_idx);
    }
...
int get_goals_count_in_radius(Drive* env, int start_grid_row, int start_grid_col,
    ...
    while (i < rel_neighbor_offsets_cnt) {
        ...
        goals_count += env->grid_map->cell_roadlanes_count[neighbor_grid_row * env->grid_map->grid_cols + neighbor_grid_col];
    }
    return goals_count;
}
...
bool get_valid_radius_curriculum_goal(Drive* env, float start_point_x, float start_point_y, float start_point_z,
    ...
    int goals_count = get_goals_count_in_radius(env, start_grid_row, start_grid_col,
                                        rel_neighbor_offsets, rel_neighbor_offsets_cnt);
    if (goals_count == 0) {
        printf("Candidate goals count is zero at (%d, %d), roadlane_cnt: %d\n", start_grid_row, start_grid_col, env->grid_map->cell_roadlanes_count[start_grid_row * env->grid_map->grid_cols + start_grid_col]);
        printf("Grid cols: %d, Grid rows: %d\n", env->grid_map->grid_cols,  env->grid_map->grid_rows);
        return false;
    }

    // No validation or rejection for very few goals count
    bool goal_validation_enabled = true;
    if (goals_count < MIN_POINTS_IN_RADIUS_TO_SAMPLE_GOAL) {
        goal_validation_enabled = false;
    }

    while(1) {
        // printf("%d goals to sample from\n", goals_count);
        int rand_goal_idx = rand() % goals_count;
        int i = 0;
        bool goal_candidate_found = false;
        // Retrieve the random goal point
        while (!goal_candidate_found) {
            int offset_row = rel_neighbor_offsets[i++];
            int offset_col = rel_neighbor_offsets[i++];
            int neighbor_grid_row = start_grid_row + offset_row;
            int neighbor_grid_col = start_grid_col + offset_col;
        ...
        if (goal_validation_enabled) {
            // Validate it so that agent is active
            float dist_to_goal = sqrtf((start_point_x - *goal_x) * (start_point_x - *goal_x) +
                                        (start_point_y - *goal_y) * (start_point_y - *goal_y));
            // printf("Sampled goal (%f, %f), distance to start (%f, %f): %f\n", *goal_x, *goal_y, start_point_x, start_point_y, dist_to_goal);
            if (dist_to_goal < env->goal_radius) {
                // Invalid goal, resample
                // printf("Resampling goal, distance to goal %f less than goal radius %f\n", dist_to_goal, env->goal_radius);
                continue;
            }

            // Reject if distance to goal is more than Curriculum Radius
            if (dist_to_goal > env->goal_curriculum_end_distance) {
                // printf("Resampling goal, distance to goal %f greater than curriculum radius %f\n", dist_to_goal, env->goal_curriculum_end_distance);
                continue;
            }
...
void seed_prng_once(void) {
    ...
    const char* env_seed = getenv("PUFFER_SEED");
    if (env_seed && *env_seed) {
        unsigned s = (unsigned)strtoul(env_seed, NULL, 10);
        srand(s);
        seeded = 1;
        return;
    }
    ...
    unsigned seed = (unsigned)(ts.tv_nsec ^ (ts.tv_sec * 1000003u) ^ getpid() ^ (uintptr_t)&seeded);
    srand(seed);
    seeded = 1;
}
...
void init(Drive* env){
    ...
    for (int i = 0; i < env->active_agent_count; i++) {
        int agent_idx = env->active_agent_indices[i];
        record_spawn_distance_to_goal(env, agent_idx, i);
    }
}
...
void c_close(Drive* env){
    ...
    // free(env->map_name);
    free(env->ini_file);
    // Free tracks_to_predict_indices if allocated
...
void compute_observations(Drive* env) {
    ...
    for(int i = 0; i < env->active_agent_count; i++) {
        ...
        if (env->goal_behavior == GOAL_GENERATE_NEW) {
            if (ego_entity->next_goal_valid) {
                float next_goal_x = ego_entity->next_goal_position_x - ego_entity->x;
                float next_goal_y = ego_entity->next_goal_position_y - ego_entity->y;
                float rel_next_goal_x = next_goal_x*cos_heading + next_goal_y*sin_heading;
                float rel_next_goal_y = -next_goal_x*sin_heading + next_goal_y*cos_heading;
                obs[base_ego_dim] = rel_next_goal_x * 0.005f;
                obs[base_ego_dim + 1] = rel_next_goal_y * 0.005f;
            } else {
                obs[base_ego_dim] = 0.0f;
                obs[base_ego_dim + 1] = 0.0f;
            }
        }
...
void c_reset(Drive* env){
    ...
    for (int x = 0; x < env->active_agent_count; x++) {
        int agent_idx = env->active_agent_indices[x];
        align_heading_to_lane(env, agent_idx);
    }
    for(int x = 0;x<env->active_agent_count; x++){
        env->logs[x] = (Log){0};
        int agent_idx = env->active_agent_indices[x];
        ...
        env->entities[agent_idx].spawn_distance_sum = 0.0f;
        env->entities[agent_idx].spawn_count = 0;
        env->entities[agent_idx].next_goal_valid = 0;
        ...
        record_spawn_distance_to_goal(env, agent_idx, x);
        compute_agent_metrics(env, agent_idx);
        if (env->goal_behavior==GOAL_GENERATE_NEW) {
            set_next_goal_preview(env, agent_idx);
        }
...
void c_step(Drive* env){
    ...
    for(int i = 0; i < env->active_agent_count; i++){
        ...
        compute_agent_metrics(env, agent_idx);
        if (env->entities[agent_idx].current_lane_idx == -1) {
            env->logs[i].missing_lane_steps += 1.0f;
        }
        ...
        if(env->entities[agent_idx].sampled_new_goal && env->goal_behavior == GOAL_GENERATE_NEW){
            promote_next_goal(env, agent_idx);
        }
