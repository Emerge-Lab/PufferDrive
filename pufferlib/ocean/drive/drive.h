...
static float score_lane_candidate(
    Drive *env,
    float agent_heading,
    int *checked_lanes,
    int *num_checked_lanes,
    int *closest_segment_index,
    GridMapEntity entity,
    float agent_x,
    float agent_y,
...
static bool check_spawn_collision(
    Drive *env,
    int num_existing_agents,
    float spawn_x,
    float spawn_y,
    float spawn_z,
    float spawn_heading,
...
static bool check_spawn_offroad(
    Drive *env,
    float spawn_x,
    float spawn_y,
    float spawn_heading,
...
static bool check_spawn_red_light_violation(
    Drive *env,
    float spawn_x,
    float spawn_y,
    float spawn_z,
    ...
    for (int i = 0; i < 4; i++) {
        corners[i][0] = spawn_x
            + (offsets[i][0] * half_length * temp_agent.cos_heading
               - offsets[i][1] * half_width * temp_agent.sin_heading);
        corners[i][1] = spawn_y
            + (offsets[i][0] * half_length * temp_agent.sin_heading
...
static int spawn_agent(Drive *env, int agent_idx, int num_agents) {
    ...
    for (int attempt = 0; attempt < MAX_SPAWN_ATTEMPTS; attempt++) {
        ...
        if (check_spawn_red_light_violation(
                env,
                spawn_x,
                spawn_y,
                spawn_z,
...
static void compute_metrics(Drive *env, int agent_idx) {
    ...
    for (int i = 0; i < list_size; i++) {
        ...
        float signed_dist_out;
        float lane_heading_out;
        float score = score_lane_candidate(
            env,
            agent->sim_heading,
            checked_lanes,
            &num_checked_lanes,
