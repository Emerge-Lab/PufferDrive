...
struct Drive {
    ...
    int use_exact_episode_seed;     // Use seed_stream_state directly for the next episode seed
    unsigned int rng_state;         // Active episode RNG state used by rand_r during simulation
    unsigned int seed_stream_state; // Persistent stream that generates one seed per episode
    unsigned int episode_seed;      // Seed selected for the current episode
    unsigned int log_episode_seed;  // Episode seed copied into eval logs for replay
    // Reward coefficients
    float reward_goal;
...
int load_map_binary(const char *filename, Drive *drive) {
    ...
    for (int i = 0; i < num_total_agents; i++) {
        ...
        if (fread(&agent->goal_gt_x, sizeof(float), 1, file) != 1) {
            fclose(file);
            return -1;
        }
        if (fread(&agent->goal_gt_y, sizeof(float), 1, file) != 1) {
            fclose(file);
            return -1;
        }
        if (fread(&agent->goal_gt_z, sizeof(float), 1, file) != 1) {
            fclose(file);
            return -1;
        }
...
static bool check_stop_line_crossing(Drive *env, Agent *agent, bool include_yellow) {
...
static int collision_check(Drive *env, int agent_idx) {
    ...
    float ego_disp = hypotf(agent->sim_x - agent->prev_x, agent->sim_y - agent->prev_y);
    ...
    for (int i = 0; i < env->num_agents; i++) {
        ...
        float other_disp = hypotf(other_agent->sim_x - other_agent->prev_x, other_agent->sim_y - other_agent->prev_y);
        float threshold = agent->radius + other_agent->radius + 0.5f + ego_disp + other_disp;
...
static bool spawn_agent(Drive *env, int agent_idx, int num_agents) {
    ...
    if (env->eval_mode) {
        ...
        spawn_width = random_uniform(env, 0.8f, 3.0f);
    }
...
static void compute_metrics(Drive *env, int agent_idx, int log_idx) {
    ...
    float distance_to_goal_gt = compute_point_to_segment_distance(
        ...
        agent->sim_x,
        agent->sim_y);
    float goal_gt_z_dist = fabsf(agent->sim_z - agent->goal_gt_z);
    if (distance_to_goal_gt < 6.0f && goal_gt_z_dist < Z_BUFFER) {
        log_agent->reached_goal_gt = 1.0f;
    }
}
...
static void compute_rewards(Drive *env, int i) {
    ...
    if (agent->metrics_array[REACHED_GOAL_IDX] > 0.0f) {
        bool final_waypoint = (agent->current_goal_idx == env->num_target_waypoints);
    ...
    log_agent->episode_return += env->rewards[i];
    ...
    float total_duration_s = env->scenario_length * env->dt;
    float reference_progress_distance = PUFFER_PROGRESS_REFERENCE_SPEED * total_duration_s;
    reference_progress_distance = fmaxf(reference_progress_distance, 1.0f);
...
// Snapshot every agent's pose at t (prev) before the move loops, for §A.5 relative-frame checks.
// Covers active, static, and expert-static agents (any index collision_check may reference).
static void snapshot_all_prev_pose(Drive *env) {
    for (int i = 0; i < env->num_total_agents; i++) {
...
void c_reset(Drive *env) {
    if (env->timestep == 0) {
        snapshot_all_prev_pose(env); // prev = curr => first-frame §A.5 sweeps are no-ops
