...
static const RewardBound REWARD_BOUNDS[NUM_REWARD_COEFS] = {
    ...
    {3.0e-3f, 3.0f, 0},    // REWARD_COEF_COLLISION       α_collision ~ U(3e-3, 3)
    {3.0e-3f, 3.0f, 0},    // REWARD_COEF_OFFROAD         α_boundary ~ U(3e-3, 3)
    {1.0e-4f, 0.1f, 1},    // REWARD_COEF_COMFORT         α_comfort ~ logU(1e-4, 0.1)
    {2.5e-4f, 2.5e-2f, 1}, // REWARD_COEF_LANE_ALIGN      α_l-align ~ logU(2.5e-4, 2.5e-2)
    {1.0e-3f, 1.0f, 0},    // REWARD_COEF_VEL_ALIGN       α_vel-align ~ U(1e-3, 1)
    ...
    {1.0e-3f, 1.0f, 0},    // REWARD_COEF_STOP_LINE       α_stop-line ~ U(1e-3, 1)
    {0.0f, 5e-5f, 0},      // REWARD_COEF_TIMESTEP        α_timestep = 2.5e-5 (fixed)
    {1.0e-3f, 1.0f, 0},    // REWARD_COEF_OVERSPEED       α_overspeed ~ U(1e-3, 1)
...
static float sample_normal(float mean, float std) {
    // Box-Muller transform on the seeded uniform stream
    float u1 = sample_uniform(0.0f, 1.0f);
    float u2 = sample_uniform(0.0f, 1.0f);
...
static void compute_agent_ttc(Drive *env, int agent_idx) {
    ...
    if (agent->removed || agent->sim_valid != 1) {
        return;
    }
...
static void add_log(Drive *env) {
    ...
    for (int i = 0; i < env->active_agent_count; i++) {
        ...
        if (!total_infractions) {
            bool scored = env->simulation_mode == SIMULATION_REPLAY ? env->logs[i].reached_goal_gt > 0.0f
                                                                    : (num_goals_reached >= 1);
            if (scored) {
                env->log.score += 1.0f;
            } else {
...
void set_active_agents(Drive *env) {
    ...
    for (int i = 0; i < env->num_total_agents && env->num_agents < max_agents; i++) {
        ...
        if (is_controlled) {
            ...
            if (is_log_replay || env->agents[i].control_state <= CONTROL_STATE_MOVING) {
                expert_static_agent_indices[env->expert_static_agent_count] = i;
                env->expert_static_agent_count++;
                env->agents[i].control_state = CONTROL_STATE_MOVING;
            }
        }
    }
...
static void compute_metrics(Drive *env, int agent_idx, int log_idx) {
    ...
    if (agent->removed || agent->stopped || agent->sim_valid != 1) {
        return;
    }
...
static void move_dynamics(Drive *env, int action_idx, int agent_idx) {
    ...
    if (env->dynamics_model == CLASSIC) {
        ...
        if (env->dynamics_noise_long_std > 0.0f) {
            acceleration += sample_normal(0.0f, env->dynamics_noise_long_std);
        }
        if (env->dynamics_noise_lat_std > 0.0f) {
            steering += sample_normal(0.0f, env->dynamics_noise_lat_std);
        }
        ...
        if (env->dynamics_noise_long_std > 0.0f) {
            j_long += sample_normal(0.0f, env->dynamics_noise_long_std);
        }
        if (env->dynamics_noise_lat_std > 0.0f) {
            j_lat += sample_normal(0.0f, env->dynamics_noise_lat_std);
        }
    ...
    if (env->dynamics_noise_speed_std > 0.0f || env->dynamics_noise_heading_std > 0.0f) {
        float speed
            = clip(agent->sim_speed_signed + sample_normal(0.0f, env->dynamics_noise_speed_std), -MAX_SPEED, MAX_SPEED);
        float heading = normalize_heading(agent->sim_heading + sample_normal(0.0f, env->dynamics_noise_heading_std));
