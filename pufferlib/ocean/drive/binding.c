...
static PyObject *my_shared(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (simulation_mode == SIMULATION_GIGAFLOW) {
        if (eval_mode) {
            ...
            int env_count = use_eval_map_indices ? num_agents / agents_per_env
                                                 : (num_agents + agents_per_env - 1) / agents_per_env - 1;
...
static int my_init(Env *env, PyObject *args, PyObject *kwargs) {
    ...
    env->use_exact_episode_seed = (bool) unpack(kwargs, "use_exact_episode_seed");
    ...
    // Seed the per-env master RNG stream; init() draws episode 0's seed from it. This replaces
    // reliance on the global srand() state for all in-env randomness.
...
static int my_log(PyObject *dict, Env *env, Log *log, float n) {
    ...
    if (env->eval_mode) {
        // Seed is an exact identifier, not a metric: set it as an integer (assign_to_dict
        // routes through float and would lose precision on a 31-bit seed).
