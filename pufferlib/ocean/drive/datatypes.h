...
#define REWARD_COEF_LANE_CENTER 5
#define REWARD_COEF_VELOCITY 6
...
int normalize_traffic_light_state(int state) {
...
int unnormalize_traffic_light_state(int norm_state) {
    if (norm_state == 0) {
        return 4; // Red
    } else if (norm_state == 1) {
        return 2; // Yellow
...
struct Agent {
    ...
    // Per-step simulation trajectory recording (for checkpoint replay)
    float *sim_traj_x;
    float *sim_traj_y;
    float *sim_traj_z;
    ...
    float metrics_array[12]; // [collision, offroad, red_light, reached_goal, lane_dist,
                             // lane_angle, comfort_violation, velocity_progress, speed_limit, avg_displacement_error,
                             // lane_aligned, lane_sin_angle]
    int collision_state;
