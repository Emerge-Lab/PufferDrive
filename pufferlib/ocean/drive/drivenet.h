...
DriveNet* init_drivenet(Weights* weights, int num_agents, int dynamics_model, int has_target) {
    ...
    if (has_target) {
        net->obs_target = calloc(num_agents*7, sizeof(float));
        net->target_encoder = make_linear(weights, num_agents, 7, input_size);
        net->target_layernorm = make_layernorm(weights, num_agents, input_size);
        net->target_encoder_two = make_linear(weights, num_agents, input_size, input_size);
        ...
        net->obs_target = NULL;
        net->target_encoder = NULL;
        net->target_layernorm = NULL;
        net->target_encoder_two = NULL;
        net->cat3 = NULL;
...
void free_drivenet(DriveNet* net) {
    ...
    if (net->has_target) {
        free(net->obs_target);
        free(net->target_encoder);
        free(net->target_layernorm);
        free(net->target_encoder_two);
        free(net->cat3);
    }
    free(net->gelu);
...
void forward(DriveNet* net, float* observations, int* actions) {
    ...
    int target_dim = net->has_target ? 7 : 0;
    ...
    if (net->has_target) {
        memset(net->obs_target, 0, net->num_agents * 7 * sizeof(float));
    }

    for (int b = 0; b < net->num_agents; b++) {
        int b_offset = b * (ego_dim + target_dim + 63*7 + 200*7);
        int target_offset = b_offset + ego_dim;
        int partner_offset = b_offset + ego_dim + target_dim;
        int road_offset = partner_offset + 63*7;
        ...
        if (net->has_target) {
            for(int i = 0; i < 7; i++) {
                net->obs_target[b * 7 + i] = observations[target_offset + i];
            }
        }
    ...
    if (net->has_target) {
        linear(net->target_encoder, net->obs_target);
        layernorm(net->target_layernorm, net->target_encoder->output);
        linear(net->target_encoder_two, net->target_layernorm->output);
    }
    ...
    if (net->has_target) {
        cat_dim1(net->cat3, net->cat2->output, net->target_encoder_two->output);
