...
from pufferlib.ocean.drive import nuplan_bridge as nb
from pufferlib.ocean.drive.drive import Drive

from pufferlib.ocean.drive.nuplan_bridge import DEFAULT_ARCH, FAR_AWAY
...
class PufferDrivePlanner(AbstractPlanner):
    ...
    def _build(self, ego_state: EgoState) -> None:
        ...
        self._env = Drive(
            ...
            scenario_length=1_000_000, resample_frequency=0, goal_on_lane=False,
            **self._arch,
        )
        self._env.reset()
        ...
        if self._debug_bev_dir:
            ...
            from pufferlib.ocean.drive.carla_cosim import BEVRenderer
    ...
    def _sync(self, ego_state: EgoState, detections: DetectionsTracks, traffic_light_data) -> None:
        ...
        # ego (slot 0): center pose + global-frame velocity
        h = float(ego_state.center.heading)
        v_local = ego_state.dynamic_car_state.center_velocity_2d
        ...
        if len(surplus):
            ...
            env.set_agent_states(surplus, far, far, far, zero, zero, zero)
