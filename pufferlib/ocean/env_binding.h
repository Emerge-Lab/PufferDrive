...
static PyObject *env_put(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (num_args != 1) {
        PyErr_SetString(PyExc_TypeError,
                        "env_put requires 1 positional argument");
        return NULL;
    }
    ...
    for (int i = 0; i < num_envs; i++) {
        ...
        if (!env) {
            PyErr_SetString(PyExc_MemoryError,
                            "Failed to allocate environment");
            Py_DECREF(kwargs);
            return NULL;
        }
        vec->envs[i] = env;
...
static PyObject *vectorize(PyObject *self, PyObject *args) {
    ...
    for (int i = 0; i < num_envs; i++) {
        ...
        if (!PyObject_TypeCheck(handle_obj, &PyLong_Type)) {
            PyErr_SetString(PyExc_TypeError,
                            "Env ids must be integers. Pass them as separate "
                            "args with *env_ids, not as a list.");
            return NULL;
        }
...
static PyObject *get_global_agent_state(PyObject *self, PyObject *args) {
    if (PyTuple_Size(args) != 5) {
        PyErr_SetString(PyExc_TypeError,
                        "get_global_agent_state requires 5 arguments");
        return NULL;
    }
    ...
    if (!PyArray_Check(x_arr) || !PyArray_Check(y_arr) ||
        ...
        PyErr_SetString(PyExc_TypeError,
                        "All output arrays must be NumPy arrays");
        return NULL;
    }
...
static PyObject *vec_get_global_agent_state(PyObject *self, PyObject *args) {
    if (PyTuple_Size(args) != 6) {
        PyErr_SetString(PyExc_TypeError,
                        "vec_get_global_agent_state requires 6 arguments");
        return NULL;
    }
    ...
    for (int i = 0; i < vec->num_envs; i++) {
        ...
        c_get_global_agent_state(drive, &x_base[offset], &y_base[offset],
                                 &z_base[offset], &heading_base[offset],
                                 &id_base[offset]);
...
static PyObject *get_ground_truth_trajectories(PyObject *self, PyObject *args) {
    if (PyTuple_Size(args) != 7) {
        PyErr_SetString(PyExc_TypeError,
                        "get_ground_truth_trajectories requires 7 arguments");
        return NULL;
    }
    ...
    int *id_data = (int *)PyArray_DATA((PyArrayObject *)id_arr);
    int *scenario_id_data =
        (int *)PyArray_DATA((PyArrayObject *)scenario_id_arr);

    c_get_global_ground_truth_trajectories(drive, x_data, y_data, z_data,
                                           heading_data, valid_data, id_data,
                                           scenario_id_data);

    Py_RETURN_NONE;
}

static PyObject *vec_get_global_ground_truth_trajectories(PyObject *self,
                                                          PyObject *args) {
    if (PyTuple_Size(args) != 8) {
        PyErr_SetString(
            PyExc_TypeError,
            "vec_get_global_ground_truth_trajectories requires 8 arguments");
        return NULL;
    }
    ...
    for (int i = 0; i < vec->num_envs; i++) {
        ...
        c_get_global_ground_truth_trajectories(
            drive, &x_base[traj_offset], &y_base[traj_offset],
            &z_base[traj_offset], &heading_base[traj_offset],
            &valid_base[traj_offset], &id_base[agent_offset],
...
static double unpack(PyObject *kwargs, char *key) {
    ...
    if (PyLong_Check(val)) {
        ...
        if (out > INT_MAX || out < INT_MIN) {
            char error_msg[100];
            snprintf(error_msg, sizeof(error_msg),
                     "Value %ld of integer argument %s is out of range", out,
                     key);
            PyErr_SetString(PyExc_TypeError, error_msg);
            return 1;
        }
        // Cast on return. Safe because double can represent all 32-bit ints
        // exactly
        return out;
    }
...
static char *unpack_str(PyObject *kwargs, char *key) {
    ...
    if (!PyUnicode_Check(val)) {
        char error_msg[100];
        snprintf(error_msg, sizeof(error_msg),
                 "Keyword argument '%s' must be a string", key);
        PyErr_SetString(PyExc_TypeError, error_msg);
        return NULL;
    }
...
static PyMethodDef methods[] = {
    ...
    {"shared", (PyCFunction)my_shared, METH_VARARGS | METH_KEYWORDS,
     "Shared state"},
    {"get_global_agent_state", get_global_agent_state, METH_VARARGS,
     "Get global agent state"},
    {"vec_get_global_agent_state", vec_get_global_agent_state, METH_VARARGS,
     "Get agent state from vectorized env"},
    {"get_ground_truth_trajectories", get_ground_truth_trajectories,
     METH_VARARGS, "Get ground truth trajectories"},
    {"vec_get_global_ground_truth_trajectories",
     vec_get_global_ground_truth_trajectories, METH_VARARGS,
