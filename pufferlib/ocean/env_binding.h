...
static PyObject *vec_reset(PyObject *self, PyObject *args) {
    ...
    int use_seed_list = PyList_Check(seed_arg);
    int seed = 0;
    if (!use_seed_list) {
        if (!PyObject_TypeCheck(seed_arg, &PyLong_Type)) {
            PyErr_SetString(PyExc_TypeError, "seed must be an integer or a list of integers");
            return NULL;
        }
        seed = PyLong_AsLong(seed_arg);
    } else if (PyList_Size(seed_arg) < vec->num_envs) {
        PyErr_SetString(PyExc_ValueError, "seed list must have at least one seed per env");
        return NULL;
    }

    for (int i = 0; i < vec->num_envs; i++) {
        if (use_seed_list) {
            vec->envs[i]->seed_stream_state = (unsigned int) PyLong_AsLong(PyList_GetItem(seed_arg, i));
        } else {
            // Assumes each process has the same number of environments
...
PyMODINIT_FUNC PyInit_binding(void) {
    ...
    PyModule_AddIntConstant(m, "MAX_TARGET_POINTS", MAX_TARGET_POINTS);
