...
static PyObject *vec_log(PyObject *self, PyObject *args) {
    ...
    if (env->eval_mode) {
        ...
        for (int i = 0; i < vec->num_envs; i++) {
            ...
            float episode_length = env->log.episode_length;
            float target_episode_length = env->log.target_episode_length;
            ...
            float target_detection_braking_margin_seconds = env->log.target_detection_braking_margin_seconds;
            float target_detection_braking_margin_valid_count = env->log.target_detection_braking_margin_valid_count;
            ...
            if (target_n > 0.0f) {
                env->log.target_episode_length = target_episode_length / target_n;
                ...
                env->log.did_target_make_progress = did_target_make_progress / target_n;
                env->log.did_target_have_at_fault_collision = did_target_have_at_fault_collision / target_n;
                env->log.target_num_goals_reached = target_num_goals_reached / target_n;
            ...
            if (target_collision_count > 0.0f) {
                ...
                env->log.target_detection_braking_margin_seconds =
            ...
            env->log.target_detection_braking_margin_valid_count = target_detection_braking_margin_valid_count;
            ...
            if (target_hit_count > 0.0f) {
                ...
                env->log.target_hit_at_fault_rate = target_hit_at_fault_count / target_hit_count;
            }
            env->log.target_hit_count = target_hit_count;
            env->log.target_hit_at_fault_count = target_hit_at_fault_count;

            float adversary_n = n - target_n;
            env->log.episode_length =
                adversary_n > 0.0f ? (episode_length - target_episode_length) / adversary_n : 0.0f;
        ...
        float episode_length = aggregate.episode_length;
        float target_episode_length = aggregate.target_episode_length;
        ...
        float target_detection_braking_margin_seconds = aggregate.target_detection_braking_margin_seconds;
        float target_detection_braking_margin_valid_count = aggregate.target_detection_braking_margin_valid_count;
        ...
        if (target_n > 0.0f) {
            aggregate.target_episode_length = target_episode_length / target_n;
            ...
            aggregate.did_target_make_progress = did_target_make_progress / target_n;
            aggregate.did_target_have_at_fault_collision = did_target_have_at_fault_collision / target_n;
            aggregate.target_num_goals_reached = target_num_goals_reached / target_n;
            ...
            aggregate.target_last_avoidable_braking_seconds_before_collision =
                target_avoidable_by_braking_collision_count > 0.0f
                    ? target_last_avoidable_braking_seconds_before_collision /
                          target_avoidable_by_braking_collision_count
                    : NO_AVOIDABLE_BRAKING_TIME;
            aggregate.target_detection_braking_margin_seconds =
                target_detection_braking_margin_valid_count > 0.0f
                    ? target_detection_braking_margin_seconds / target_detection_braking_margin_valid_count
                    : FIRST_DETECTED_NOT_DETECTED;
        ...
        aggregate.target_detection_braking_margin_valid_count = target_detection_braking_margin_valid_count;
        ...
        if (target_hit_count > 0.0f) {
            ...
            aggregate.target_hit_at_fault_rate = target_hit_at_fault_count / target_hit_count;
        }
        aggregate.target_hit_count = target_hit_count;
        aggregate.target_hit_at_fault_count = target_hit_at_fault_count;
...
PyMODINIT_FUNC PyInit_binding(void) {
    ...
    PyModule_AddIntConstant(m, "CONTROLLER_NUPLAN_IDM", CONTROLLER_NUPLAN_IDM);
