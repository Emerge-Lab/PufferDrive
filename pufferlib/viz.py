...
def generate_interactive_replay(scenario, replay, filename="replay.html"):
    ...
    metadata = {
        ...
        "target_features": int(
            binding.STATIC_TARGET_FEATURES if target_type == "static" else binding.DYNAMIC_TARGET_FEATURES
        ),
        "scales": scales,
...
<body>
    ...
    <script>
        ...
        async function initReplay() {
            ...
            const compressed = await (await fetch('data:application/octet-stream;base64,' + B64_PAYLOAD)).blob();
        ...
        function decodeObs(frame, slot) {
            if (slot < 0 || slot >= H.active_count || !H.obs_dim) return null;
            const base = (frame * H.active_count + slot) * H.obs_dim, obs = C.obs, Q = H.obs_scale, RF = H.road_features, TF = H.traffic_features;
            ...
            const lanesStart = p; p += H.lane_count * RF;
            const boundsStart = p; p += H.boundary_count * RF;
            const trafficStart = p;
            const rot = (x,y) => [-y,x];
            const zero = (off,n) => { for(let i=0;i<n;i++) if(obs[off+i] !== 0) return false; return true; };
            const roads = (start,count,poolName) => { const out=[]; for(let i=0;i<count;i++){ const o=start+i*RF; if(zero(o,RF)) continue; let xy=rot(obs[o]*Q,obs[o+1]*Q), cs=rot(obs[o+5]*Q,obs[o+6]*Q); out.push([xy[0],xy[1],obs[o+3]*Q*H.scales.road_length_to_position,obs[o+4]*Q*H.scales.road_width_to_position,cs[0],cs[1],poolAt(poolName,frame,slot,i)]); } return out; };
