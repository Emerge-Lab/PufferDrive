...
`;
        ...
        function draw(force=false) {
            ...
            for(const a of displayedAgents){ ctx.save(); ctx.translate(a.x,a.y); ctx.rotate(a.h); drawAgentBody(a, darkMode?'#fff':'#111'); drawPerturbationOutlines(a); ctx.restore(); ctx.save(); ctx.translate(a.x,a.y); if(isEgoCam && target) ctx.rotate(-Math.PI/2 + target.h); else ctx.scale(1,-1); const isAdversary=adversaryIds.has(a.id); const isLossAdversary=a.id===egoCollisionLossAdversaryId; ctx.fillStyle=isLossAdversary?LOSS_ADVERSARY_COLOR:(isAdversary?ADVERSARY_COLOR:colors.text); ctx.font=isAdversary?'bold '+(14/cam.z)+'px system-ui':'600 '+(14/cam.z)+'px system-ui'; ctx.textAlign='center'; ctx.fillText(isLossAdversary?a.id+' [LOSS ADV]':(isAdversary?a.id+' [ADV]':String(a.id)),0,(isEgoCam && target)?a.w/2+.5:-a.w/2-.5); ctx.restore(); if(a.id === followedId){ ctx.save(); ctx.translate(a.x,a.y); ctx.strokeStyle=colors.accent; ctx.lineWidth=3/cam.z; ctx.beginPath(); ctx.arc(0,0,Math.max(a.l,a.w)*1.2,0,7); ctx.stroke(); ctx.restore(); } }
