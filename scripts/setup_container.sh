#!/bin/bash
# Setup script for PufferDrive (puffer-4) Singularity container environment.
# Creates an overlay with all dependencies for running on HPC clusters with
# older glibc versions.
#
# Usage:
#   1. Create an overlay: ./scripts/setup_container.sh create-overlay
#   2. Install dependencies: sbatch --gres=gpu:1 --wrap "./scripts/setup_container.sh install"
#   3. Rebuild C extension: sbatch --gres=gpu:1 --wrap "./scripts/setup_container.sh rebuild"
...
install_deps() {
    ...
    # build.sh handles that in a separate step.
    echo "=== Installing PufferDrive Python package ==="
    cd "$PROJECT_ROOT"
    pip3 install -e .

    # Cluster-side helpers not listed in pyproject.toml dependencies.
    echo "=== Installing cluster helpers ==="
    pip3 install submitit pyyaml onnx onnxruntime

    # Build the C / CUDA extension.
    echo "=== Building _C extension via build.sh ==="
    ./build.sh

    echo "=== Setup complete ==="
    python3 -c "import torch; print(f'PyTorch: {torch.__version__}, CUDA: {torch.cuda.is_available()}')"
    python3 -c "from pufferlib import _C; print(f'_C loaded, gpu={_C.gpu}, precision_bytes={_C.precision_bytes}')"
}
...
case "${1:-}" in
    ...
    rebuild)
        ...
        ;;
    *)
        echo "PufferDrive (puffer-4) Container Setup"
        echo ""
