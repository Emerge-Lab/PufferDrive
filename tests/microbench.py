...
def log_coeffs_and_values(gate, hidden):
    ...
    log_tilde_h = torch.where(
        hidden >= 0, (torch.nn.functional.relu(hidden) + 0.5).log(), -torch.nn.functional.softplus(-hidden)
    )
...
def test_sample_logits():
    ...
    rich.print(
        f"\tLogprobs = log_softmax[action]: [{match_color}]{logprob_match} (max diff: {logprob_max_diff:.2e})[/{match_color}]"
    )
