...
from data_utils.generate_carla_pdm import DATASET_NAME, generate_map_segments
...
TEST_AGENT_COUNT = 8
TEST_TRANSITION_COUNT = 6
TEST_SEGMENT_TRANSITION_COUNT = 3
TEST_SEED = 42
...
def _replay_drive(binary_path):
    return Drive(
        ...
        dt=0.1,
        scenario_length=TEST_SEGMENT_TRANSITION_COUNT + 1,
...
def _assert_replay_matches_binary(binary_path, generated_data):
    ...
    try:
        ...
        for sample_idx in range(TEST_SEGMENT_TRANSITION_COUNT + 1):
            if sample_idx > 0:
...
def test_regents_profile_uses_all_generated_carla_pdm_segments():
    generation = load_generation_config(REGENTS_CONFIG, "regents_carla_generated_pdm")

    assert generation["seed"] == 42
    assert generation["scenario_count"] == 80
    assert generation["num_workers"] == 24
    assert generation["horizon_transition_count"] == 100
    ...
    assert generation["env"]["map_dir"] == "pufferlib/resources/drive/binaries/carla_generated_pdm"
    assert generation["env"]["num_maps"] == 80
    assert generation["env"]["scenario_length"] == 101
    assert generation["env"]["resample_frequency"] == 101
...
def test_town01_pdm_generation_is_continuous_deterministic_atomic_and_replayable(tmp_path):
    first_directory = tmp_path / "first"
    second_directory = tmp_path / "second"
    summaries = generate_map_segments(
        TOWN01_MAP,
        first_directory,
        agent_count=TEST_AGENT_COUNT,
        transition_count=TEST_TRANSITION_COUNT,
        segment_transition_count=TEST_SEGMENT_TRANSITION_COUNT,
        seed=TEST_SEED,
    )
    source_data = read_bin(TOWN01_MAP)
    segment_paths = sorted(first_directory.glob("*.bin"))
    segments = [read_bin(path) for path in segment_paths]

    assert [path.name for path in segment_paths] == [
        "opendrive__Town01__segment_00.bin",
        "opendrive__Town01__segment_01.bin",
    ]
    assert len(summaries) == 2
    for segment_idx, (summary, path, segment) in enumerate(zip(summaries, segment_paths, segments)):
        expected_scenario_id = f"Town01_seed_42_segment_{segment_idx:02d}"
        assert summary.output_path == path
        assert summary.agent_count == TEST_AGENT_COUNT
        assert summary.sample_count == TEST_SEGMENT_TRANSITION_COUNT + 1
        assert summary.scenario_id == expected_scenario_id
        assert len(summary.sha256) == 64
        assert _decoded(segment["scenario_id"]) == expected_scenario_id
        assert _decoded(segment["dataset_name"]) == DATASET_NAME
        assert segment["log_length"] == TEST_SEGMENT_TRANSITION_COUNT + 1
        assert segment["log_dt"] == pytest.approx(0.1)
        assert segment["roads"] == source_data["roads"]
        assert segment["objects"] == source_data["objects"]
        assert segment["lane_graph"] == source_data["lane_graph"]
        assert segment["has_phase_section"] == source_data["has_phase_section"]
        assert segment["has_width_section"] == source_data["has_width_section"]

        assert len(segment["agents"]) == TEST_AGENT_COUNT
        for agent_idx, agent in enumerate(segment["agents"]):
            assert agent["id"] == agent_idx
            assert agent["type"] == binding.AGENT_TYPE_VEHICLE
            assert agent["T"] == TEST_SEGMENT_TRANSITION_COUNT + 1
            assert agent["route"]
            assert agent["route_gt_len"] == len(agent["route"])
            assert agent["goal"] == tuple(agent["cols"][name][-1] for name in ("x", "y", "z"))
            assert tuple(agent["cols"]["valid"]) == (1,) * (TEST_SEGMENT_TRANSITION_COUNT + 1)
            for column_name in ("x", "y", "z", "h", "vx", "vy", "len", "wid", "hgt"):
                values = agent["cols"][column_name]
                assert len(values) == TEST_SEGMENT_TRANSITION_COUNT + 1
                assert all(math.isfinite(value) for value in values)

        assert len(segment["traffic"]) == len(source_data["traffic"])
        for generated_traffic, source_traffic in zip(segment["traffic"], source_data["traffic"]):
            ...
            if generated_traffic["type"] == binding.TRAFFIC_CONTROL_TYPE_TRAFFIC_LIGHT:
                assert len(generated_traffic["states"]) == TEST_SEGMENT_TRANSITION_COUNT + 1
            else:
                assert generated_traffic["states"] == source_traffic["states"]

    for first_agent, second_agent in zip(segments[0]["agents"], segments[1]["agents"]):
        for column_name in ("x", "y", "z", "h", "vx", "vy", "len", "wid", "hgt", "valid"):
            assert first_agent["cols"][column_name][-1] == second_agent["cols"][column_name][0]
    for first_traffic, second_traffic in zip(segments[0]["traffic"], segments[1]["traffic"]):
        if first_traffic["type"] == binding.TRAFFIC_CONTROL_TYPE_TRAFFIC_LIGHT:
            assert first_traffic["states"][-1] == second_traffic["states"][0]

    assert all(agent["cols"]["vx"][0] == 0.0 for agent in segments[0]["agents"])
    assert all(agent["cols"]["vy"][0] == 0.0 for agent in segments[0]["agents"])
    assert any(math.hypot(agent["cols"]["vx"][0], agent["cols"]["vy"][0]) > 0.0 for agent in segments[1]["agents"])

    second_summaries = generate_map_segments(
        TOWN01_MAP,
        second_directory,
        agent_count=TEST_AGENT_COUNT,
        transition_count=TEST_TRANSITION_COUNT,
    ...
    expected_bytes = [summary.output_path.read_bytes() for summary in second_summaries]
    assert [path.read_bytes() for path in segment_paths] == expected_bytes

    segment_paths[0].unlink()
    segment_paths[1].write_bytes(b"different existing output")
    with pytest.raises(FileExistsError, match="--overwrite"):
        generate_map_segments(
            TOWN01_MAP,
            first_directory,
            agent_count=TEST_AGENT_COUNT,
            transition_count=TEST_TRANSITION_COUNT,
    ...
    assert segment_paths[1].read_bytes() == b"different existing output"
    ...
    for path, segment in zip(segment_paths, segments):
        _assert_replay_matches_binary(path, segment)


def test_generation_rejects_non_divisible_segment_length(tmp_path):
