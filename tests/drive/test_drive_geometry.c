...
static int test_stop_line_red_light_gating(void) {
    ...
    EXPECT_TRUE(check_stop_line_crossing(&env, &agent, false, true));

    state[0] = TRAFFIC_CONTROL_STATE_GREEN;
    EXPECT_FALSE(check_stop_line_crossing(&env, &agent, false, true));
    state[0] = TRAFFIC_CONTROL_STATE_YELLOW;
    EXPECT_FALSE(check_stop_line_crossing(&env, &agent, false, true));
    EXPECT_TRUE(check_stop_line_crossing(&env, &agent, true, false)); // yellow counts when spawn-mode flag is set
...
static int test_stop_line_stationary_on_red(void) {
    ...
    // Box mode (spawn checks) flags a stationary overlap; center mode only flags the crossing step
    EXPECT_TRUE(check_stop_line_crossing(&env, &agent, false, false));
    EXPECT_FALSE(check_stop_line_crossing(&env, &agent, false, true));
    return 0;
}
