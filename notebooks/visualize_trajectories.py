...
for mid in range(len(map_files)):
    ...
    if mask.sum() > 0:
        axes[1].hist(traj_lengths[mask], bins=30, alpha=0.5,
                     label=f'Map {mid}: {Path(str(map_files[mid])).stem}')
...
def draw_map_with_trajectory(agent_idx):
    ...
    if length > 1:
        ...
        for t in range(0, length, arrow_step):
            dx = np.cos(h[t]) * 2
            dy = np.sin(h[t]) * 2
            ax.arrow(x[t], y[t], dx, dy, head_width=0.5, head_length=0.3,
                     fc='cyan', ec='cyan', alpha=0.7, zorder=7)
    ...
    ax.set_title(f'Agent {agent_idx} | Map {mid} ({Path(str(map_files[mid])).stem}) | '
                 f'Length: {length} steps', fontsize=13)
    ax.set_xlabel('x (m)')
    ax.set_ylabel('y (m)')
    if length > 1:
...
def make_trajectory_video(agent_idx, follow_agent=True, window_size=60):
    ...
    def animate(frame):
        ...
        heading_arrow = ax.arrow(x[t], y[t], dx, dy, head_width=1.0, head_length=0.5,
                                 fc='red', ec='red', zorder=7)
        ...
        if t > 0:
            ...
            spd = 0
        title.set_text(f'Agent {agent_idx} | Step {t}/{length} | '
                       f'Speed: {spd:.1f} m/s')

        return trail_line, car_marker, heading_arrow

    anim = FuncAnimation(fig, animate, init_func=init, frames=length,
...
agent_slider = widgets.IntSlider(
    value=0, min=0, max=len(agents_sorted) - 1, step=1,
    description='Rank:',
...
def update_static(change):
    ...
    agent_label.value = (f"Agent {idx} | Map {map_ids[idx]} | "
                         f"Length {traj_lengths[idx]} steps")
    with output:
...
print(f"Animating agent {agent_idx} (rank {AGENT_RANK}), "
      f"length {traj_lengths[agent_idx]} steps, map {map_ids[agent_idx]}")
